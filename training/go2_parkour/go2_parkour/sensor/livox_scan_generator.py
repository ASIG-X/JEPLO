...
class LivoxScanGenerator:
    ...
    def __init__(self, name, num_envs, device, horizontal_fov_deg, vertical_fov_deg):
        if name in self.livox_lidar_params:
            ...
            try:
                ...
                azimuth = (self.ray_angles[:, 0] + np.pi) % (2.0 * np.pi) - np.pi
                elevation = self.ray_angles[:, 1]
                fov_mask = (
                    (azimuth >= np.deg2rad(horizontal_fov_deg[0]))
                    & (azimuth <= np.deg2rad(horizontal_fov_deg[1]))
                    & (elevation >= np.deg2rad(vertical_fov_deg[0]))
                    & (elevation <= np.deg2rad(vertical_fov_deg[1]))
                )
                self.n_samples = round(self.n_samples * fov_mask.mean())
                self.ray_angles = self.ray_angles[fov_mask]
        ...
        # index per env
        self.curr_start_index = torch.zeros(self.num_envs, dtype=torch.long, device=device)
