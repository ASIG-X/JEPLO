...
class OnPolicyRunnerTeacher:
    ...
    def __init__(
        ...
        # no separate privileged observation (everything included in obs vector)
        self.privileged_obs_type = None
        num_privileged_obs = num_obs
        print("on policy runner:")
        print(f"num_obs: {num_obs}, num_privileged_obs: {num_privileged_obs}")

        # get attributes defining observation structure
        ...
        print(f"Using policy class: {ActorCriticTeacher}")
        policy = ActorCriticTeacher(
        ...
        alg_class = PPO
        self.alg = alg_class(policy, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg)
        print(f"Using algorithm class: {alg_class}")
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35, wandb_run=None):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            if self.alg.rnd:
                wandb_run.log({"Rnd/mean_extrinsic_reward": statistics.mean(locs["erewbuffer"])}, step=locs["it"])
                wandb_run.log({"Rnd/mean_intrinsic_reward": statistics.mean(locs["irewbuffer"])}, step=locs["it"])
                wandb_run.log({"Rnd/weight": self.alg.rnd.weight}, step=locs["it"])
            # everything else
            wandb_run.log({"Train/mean_reward": statistics.mean(locs["rewbuffer"])}, step=locs["it"])
            wandb_run.log({"Train/mean_episode_length": statistics.mean(locs["lenbuffer"])}, step=locs["it"])
    ...
    def export_policy_onnx(self, export_dir: str, filename: str = "policy.onnx"):
        """Export the teacher policy to ONNX with 3 separate inputs.

        Inputs:
            proprio:     (B, obs_proprio_dim)       — current proprioception
            hist_obs:    (B, obs_proprio_hist_dim)   — proprioceptive history
            height_scan: (B, obs_height_scan_dim)    — height scan
        Output:
        ...
        class _TeacherPolicyONNX(torch.nn.Module):
            def __init__(self, policy):
                super().__init__()
                self.actor = copy.deepcopy(policy.actor)
                self.proprio_hist_encoder = copy.deepcopy(policy.proprio_hist_encoder)
                self.height_map_encoder = copy.deepcopy(policy.height_map_encoder)

            def forward(self, proprio, hist_obs, height_scan):
                obs = torch.cat(
                    [proprio, self.proprio_hist_encoder(hist_obs), self.height_map_encoder(height_scan)],
                    dim=-1,
                )
                return self.actor(obs)

        exporter = _TeacherPolicyONNX(policy).to("cpu").eval()

        dummy_proprio = torch.zeros(1, policy.obs_proprio_dim)
        dummy_hist = torch.zeros(1, policy.obs_hist_dim)
        dummy_height = torch.zeros(1, policy.obs_height_scan_dim)

        onnx_path = os.path.join(export_dir, filename)
        torch.onnx.export(
            exporter,
            (dummy_proprio, dummy_hist, dummy_height),
            onnx_path,
            export_params=True,
            opset_version=11,
            input_names=["proprio", "hist_obs", "height_scan"],
            output_names=["actions"],
            dynamic_axes={},
        )

        print(f"\nTeacher policy exported to {onnx_path}")
        print(
            f"  Inputs:  proprio ({policy.obs_proprio_dim}), "
            f"hist_obs ({policy.obs_hist_dim}), "
            f"height_scan ({policy.obs_height_scan_dim})"
        )
        print(f"  Output:  actions ({self.env.num_actions})")
