...
def store_code_state(
    ...
    if launch_command is not None:
        command_file_name = os.path.join(git_log_dir, "training_command.txt")
        if not os.path.isfile(command_file_name):
            with open(command_file_name, "x", encoding="utf-8") as f:
                content = (
                    f"--- working directory ---\n{launch_cwd or ''}"
                    f"\n\n--- command ---\n{launch_command}\n"
                )
                f.write(content)
            file_paths.append(command_file_name)
