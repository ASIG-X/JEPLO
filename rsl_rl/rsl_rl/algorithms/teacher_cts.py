...
class TeacherCTS:
    ...
    def __init__(
        ...
        print(
            f"  schedule: teacher-only until {self.warmup_end_iteration}, "
            f"full CTS from {self.full_cts_iteration}"
        )
        print("=" * 50)
    ...
    def set_iteration(self, iteration):
        ...
        if iteration < self.warmup_end_iteration:
            ...
            progress = (iteration - self.warmup_end_iteration) / (
                self.full_cts_iteration - self.warmup_end_iteration
            )
        else:
            progress = 1.0
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if student_samples > 0:
                surrogate_loss = (
                    surrogate_loss
                    + self.student_surrogate_weight * surrogate_losses[teacher_samples:].mean()
                )
