...
#include "mid360_lidar.h"
#include "param.h"
#include "simulate.h"
#include "terrain_switcher.h"
...
class ElasticBand {
  public:
    ...
    void Advance(std::vector<double> x, std::vector<double> dx) {
        ...
        double distance =
            sqrt(delta_x[0] * delta_x[0] + delta_x[1] * delta_x[1] + delta_x[2] * delta_x[2]);
...
void PhysicsThread(
    mj::Simulate *sim, const char *filename, std::shared_ptr<Mid360LidarPublisher> lidar,
    std::shared_ptr<TerrainSwitcher> terrain_switcher) {
    ...
    PhysicsLoop(*sim, lidar.get(), terrain_switcher.get());
...
TerrainSwitcher *terrain_switcher = nullptr;

void user_key_cb(GLFWwindow *window, int key, int scancode, int act, int mods) {
    // Preserve MuJoCo's built-in shortcuts and UI handling (Tab, Shift+Tab,
    // Space, Backspace, camera keys, and so on).
    if (mujoco_key_cb) {
        mujoco_key_cb(window, key, scancode, act, mods);
    }

    if (act == GLFW_PRESS) {
        bool terrain_key = false;
        if ((mods & GLFW_MOD_CONTROL) && terrain_switcher) {
            terrain_key = terrain_switcher->queueKey(key);
        }

        if (!terrain_key && param::config.enable_elastic_band == 1) {
            if (key == GLFW_KEY_9) {
...
int main(int argc, char **argv) {
    ...
    std::shared_ptr<Mid360LidarPublisher> lidar;
    if (param::config.enable_lidar) {
        if (param::config.lidar_pattern.empty()) {
            param::config.lidar_pattern = proj_dir / "assets" / "mid360.npy";
        }
    ...
    auto terrain = std::make_shared<TerrainSwitcher>();
    terrain_switcher = terrain.get();

    std::thread unitree_thread(UnitreeSdk2BridgeThread, nullptr);

    // Depth camera: initialize and set up render callback (enabled via --depth / -d)
    if (param::config.enable_depth) {
        auto depth_camera = std::make_shared<DepthCamera>();
        sim->render_callback = [&sim, depth_camera]() {
            if (m && d) {
                // Re-initialize if model changed (e.g. drag-and-drop reload)
                if (!depth_camera->isInitialized() || depth_camera->lastModel() != m) {
                    depth_camera->init(m);
                }
                depth_camera->renderFrame(*sim, m, d, sim->platform_ui->mjr_context());
            }
        };
    }
    ...
    std::thread physicsthreadhandle(
        &PhysicsThread, sim.get(), param::config.robot_scene.c_str(), lidar, terrain);
    // start simulation UI loop (blocking call)
    GLFWwindow *window = static_cast<mj::GlfwAdapter *>(sim->platform_ui.get())->window_;
    mujoco_key_cb = glfwSetKeyCallback(window, user_key_cb);
    sim->RenderLoop();
